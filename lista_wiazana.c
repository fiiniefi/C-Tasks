...
void rotacja(kolejka ktory, int n);
...
void od_gory(kolejka ktory, kolejka dokad, int n);
...
void interfejs()
{
    ...
    while (c!='.')
    {
        ...
        switch(c)
        {
            ...
            if (pomocniczy)
            {
                rotacja(pomocniczy, npom);
                if (pomocniczy != pierwszy)
                    od_gory(pierwszy, pomocniczy, n);
            }
            break;
...
void rotacja(kolejka zamiana, int n)
{
    if (n == 1)
        return;
    rotacja(zamiana->A, n/2);
    rotacja(zamiana->B, n/2);
    rotacja(zamiana->C, n/2);
    rotacja(zamiana->D, n/2);
...
void czy_jednobarwny_i_szachownica(kolejka ktory)
{
    if (ktory->A->czy_liczba == 0 && ktory->B->czy_liczba == 0 && ktory->C->czy_liczba == 0 && ktory->D->czy_liczba == 0)
    {
...
void od_gory(kolejka ktory, kolejka dokad, int n)
{
    ...
    od_gory(ktory->A, dokad, n/2);
    od_gory(ktory->B, dokad, n/2);
    od_gory(ktory->C, dokad, n/2);
    od_gory(ktory->D, dokad, n/2);
